...
首次安装时，安装流程会以 hostname 生成建议设备名，并尽量**预填到输入缓冲**（共享模块
`clipboard_relay_shared.prompt_device_id`：macOS 用标准库 readline；Windows 在装有
`pyreadline3` 时同样预填，否则退化为括号提示）。用户可直接回车确认或就地编辑。服务端注册
成功后，Agent 会把规范化后的 `device_id` 持久化到本地配置；后续启动会复用该身份，不会根据
用户名或变化后的 hostname 重新生成。
