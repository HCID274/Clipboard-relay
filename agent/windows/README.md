...
安装脚本会先执行交互式注册。配置中没有 `device_id` 时，Agent 会把从 hostname 生成的建议名
**尽量预填进输入行**（若已安装可选依赖 `pyreadline3`，体验与 macOS 一致；否则退化为
`设备名称 [建议名]:` 提示，直接回车即采用建议名）。注册成功后，最终 ID 会保存到
`config.json`，计划任务才会创建。
