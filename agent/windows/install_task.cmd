...
:ensure_password
"%PYTHON%" -c "import sys; from pathlib import Path; sys.path.insert(0, sys.argv[1]); import agent; sys.exit(agent.password_setup_status(Path(sys.argv[2])))" "%PROJECT_DIR%" "%CONFIG_PATH%"
set "PASSWORD_STATUS=%ERRORLEVEL%"

if "%PASSWORD_STATUS%"=="1" exit /b 0
if "%PASSWORD_STATUS%"=="2" goto :config_error
if not "%PASSWORD_STATUS%"=="0" exit /b 1
