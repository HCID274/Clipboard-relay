...
服务端每 0.75 秒最多为每台在线设备发起一次探测，且同一设备同时最多存在一个未完成的
`ping`。探测在 0.75 秒内没有匹配 `pong`，或者 ping 发送失败时，服务端会把该 Agent 从在线
连接表中移除、持久化最后活跃时间、以 `1011` 关闭该连接并向浏览器推送离线快照；服务端不会只
把 `latency_ms` 置空后继续把半开连接显示为在线。
