...
Agent 的受保护 HTTP 请求和 `/ws/agent` WebSocket 连接都携带
`X-API-Key: <共享密码>`。服务端使用 `hmac.compare_digest()` 比较凭据，密码不会出现在
URL 中。生产环境必须使用 HTTPS/WSS，而且日志不得记录密码。

浏览器页面不能为 WebSocket 握手设置 `X-API-Key` 请求头，因此页面先向同源
`POST /api/ui-ticket` 发送带 `X-API-Key` 的 HTTP 请求。服务端成功校验后返回一个一次性、
60 秒有效的随机票据；页面只把这个短期票据放入 `/ws/ui?ticket=...`，绝不会把长期共享密码
放入 URL。票据使用一次后立即失效，浏览器每次重连和每次密码变更都会重新换取票据。
...
`latency_ms` 是**服务器到该 Agent** 的应用层往返时延（毫秒）。服务端定期向在线 Agent
发送 `{"type":"ping"}`，Agent 收到后立即回复 `{"type":"pong"}`；列表接口返回最近一次
成功测得的缓存值。离线设备、尚未测到或测速超时的设备为 `null`。该字段**不写盘**。

### `POST /api/ui-ticket`

浏览器页面使用该接口为 UI WebSocket 换取短期票据。该接口接受
`X-API-Key: <共享密码>`，成功时返回 `200 {"ticket":"<随机票据>"}`。密码错误与其他受保护
HTTP 接口相同，返回 `401`；服务端未配置密码时返回 `500`。该票据仅能用于同源浏览器的
`/ws/ui` 建连，不能用于 Agent 或其他 HTTP 接口。
...
`200 {"ok":true}` 表示服务端已经把消息交给当前 WebSocket 连接，并不表示操作系统剪贴板已经
写入成功。macOS 和 Windows Agent 在本地写入失败后会发送
`{"type":"clipboard_report","status":"failed"}`，服务端会记录该失败；当前 HTTP 接口为了兼容
既有调用方，不会等待这一异步回执。
...
Agent 在本地剪贴板写入失败后可以向服务端发送下列失败回执；这条回执仅用于服务端日志，
不会改变兼容接口 `POST /api/send` 的即时响应：

```json
{"type":"clipboard_report","status":"failed"}
```
...
服务端每 1 秒最多为每台在线设备发起一次探测，且同一设备同时最多存在一个未完成的
`ping`。每次探测对 ping 发送和匹配 `pong` 共享 3 秒超时预算。单次超时或发送失败只会清空
`latency_ms` 并累计失败次数，以免跨境网络抖动误杀在线设备；连续 3 次失败后，服务端才会把该
Agent 从在线连接表中移除、持久化最后活跃时间、以 `1011` 关闭该连接并向浏览器推送离线快照。
...
```text
wss://当前页面主机/ws/ui?ticket=<一次性短期票据>
```

服务端拒绝缺失、过期或已使用票据，并以 `1008` 关闭连接。服务端接受连接后立即发送一条全量
设备快照；注册、删除、Agent 连接、Agent 断开、延迟成功更新和延迟超时下线都会发送新的全量快照。
每条快照带有单调递增的 `version`，浏览器必须忽略版本小于已渲染版本的消息，因此并发发送或重连
不会造成状态倒退。
...
服务端最多维护 32 个 UI 客户端。每个客户端只有一个容量为一的发送队列，因此慢浏览器只能丢弃
旧快照并保留最新快照，慢浏览器不会阻塞其他浏览器、Agent 连接或设备文件锁。设备状态快照会在
释放 `device_lock` 后投递，服务端不会在持有设备文件锁时等待浏览器网络发送。
